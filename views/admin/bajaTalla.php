            ...
            <div class="column">
				<div class="container">
							<form action="<?php echo $_SERVER['PHP_SELF']; ?>" METHOD="post">
									...
									if($conexion)
										{
											...
											echo'<label class="input-group-text" for="inputGroupSelect01">Id de la talla a consultar</label>';
											echo'<select class="form-select" id="inputGroupSelect01"  name="id_TallaC" required>';
											echo'<option selected>Selecciona una opción</option>';
											while($tupla=mysqli_fetch_array($registros))
											{
												echo "<option value=".$tupla['id_Talla'].">".$tupla['id_Talla']."</option>";
											}
								...
								<center><button type="submit" name="enviarCT" class="btn btn-default btn-primary">Consultar Talla</button><br></center>
								<br>
<?php
include("configtienda.php");

if(isset($_POST['enviarCT'])){
	if(isset($_POST['id_TallaC']))
	{
			$id_Talla=$_POST['id_TallaC'];
			...
			if($conexion)
			{
				...
				$query="SELECT * FROM talla WHERE id_Talla='".$id_Talla."';";
				$registros=mysqli_query($conexion,$query) or die("error query ".mysqli_error());
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" METHOD="post">
									...
									if($conexion)
										{
											...
											echo'<div class="input-group mb-3">';
											echo'<label class="input-group-text" for="inputGroupSelect01">Id de la talla a eliminar</label>';
											echo'<select class="form-select" id="inputGroupSelect01"  name="id_Talla" required>';
											echo'<option selected>Selecciona una opción</option>';
											while($tupla=mysqli_fetch_array($registros))
											{
												echo "<option value=".$tupla['id_Talla'].">".$tupla['id_Talla']."</option>";
								...
								?>
								<center><button type="submit" name="enviarBT" class="btn btn-default btn-primary">Eliminar Talla</button><br></center>
								<br>
<?php
include("configtienda.php");
								
if(isset($_POST['enviarBT'])){
	if(isset($_POST['id_Talla']))
	{
		...
		if($conexion)
		{
			...
			$query="DELETE FROM talla WHERE id_Talla='".$id_Talla."';";

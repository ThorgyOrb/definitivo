            ...
            <div class="column">
				<div class="container">
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" METHOD="post">
							...
							if($conexion)
								{
									...
									echo'<label class="input-group-text" for="inputGroupSelect01">Id del Proveedor a consultar</label>';
									echo'<select class="form-select" id="inputGroupSelect01"  name="id_ProveedorC" required>';
									echo'<option selected>Selecciona una opción</option>';
									while($tupla=mysqli_fetch_array($registros))
									{
										echo "<option value=".$tupla['id_Proveedor'].">".$tupla['id_Proveedor']."</option>";
									}
						...
						?>
						<center><button type="submit" name="enviarCP" class="btn btn-default btn-primary">Consultar Proveedor</button><br></center>
						<br>
<?php
include("configtienda.php");

if(isset($_POST['enviarCP'])){
	if(isset($_POST['id_ProveedorC']))
	{
		$id_Proveedor=$_POST['id_ProveedorC'];
		...
		if($conexion)
		{
			...
			echo '<th scope="col">Nombre Proveedor</th>';
			echo '</thead>';
			echo '<tbody>';
			while($tupla=mysqli_fetch_array($registros))
			{
				echo "<tr><td>".$tupla['id_Proveedor']."</td>";
				echo "<td>".$tupla['nombre_Proveedor']."</td></tr>";
			}
			echo '</tbody>';
			echo '</table>';
			mysqli_close($conexion);
...
?>
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" METHOD="post">
							...
							if($conexion)
								{
									...
									echo'<div class="input-group mb-3">';
									echo'<label class="input-group-text" for="inputGroupSelect01">Id del Proveedor a eliminar</label>';
									echo'<select class="form-select" id="inputGroupSelect01"  name="id_Proveedor" required>';
									echo'<option selected>Selecciona una opción</option>';
									while($tupla=mysqli_fetch_array($registros))
									{
										echo "<option value=".$tupla['id_Proveedor'].">".$tupla['id_Proveedor']."</option>";
						...
						?>
						<center><button type="submit" name="enviarBP" class="btn btn-default btn-primary">Eliminar Proveedor</button><br></center>
						<br>
<?php
include("configtienda.php");

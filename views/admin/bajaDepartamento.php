					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" METHOD="post">
						...
						<?php
							...
							if($conexion)
							{
								...
								echo'<label class="input-group-text" for="inputGroupSelect01">Id del departamento a consultar</label>';
								echo'<select class="form-select" id="inputGroupSelect01"  name="id_DepartamentoC" required>';
								echo'<option selected>Selecciona una opción</option>';
								while($tupla=mysqli_fetch_array($registros))
								{
									echo "<option value=".$tupla['id_Departamento'].">".$tupla['id_Departamento']."</option>";
								}
						...
						?>
						<center><button type="submit" name="enviarCD" class="btn btn-default btn-primary">Consultar Departamento</button><br></center>
						<br>
<?php
include("configtienda.php");

if(isset($_POST['enviarCD'])){
	if(isset($_POST['id_DepartamentoC']))
	{
			$id_Departamento=$_POST['id_DepartamentoC'];
			...
			if($conexion)
			{
				...
				echo '<th scope="col">Nombre Departamento</th>';
				echo '</thead>';
				echo '<tbody>';
				...
				{
					echo "<tr><td>".$tupla['id_Departamento']."</td>";
					echo "<td>".$tupla['nombre_Departamento']."</td></tr>";
				}
...
?>
									...
									if($conexion)
										{
											...
											echo'<div class="input-group mb-3">';
											echo'<label class="input-group-text" for="inputGroupSelect01">Id del departamento a eliminar</label>';
											echo'<select class="form-select" id="inputGroupSelect01"  name="id_DepartamentoE" required>';
											echo'<option selected>Selecciona una opción</option>';
											while($tupla=mysqli_fetch_array($registros))
											{
												echo "<option value=".$tupla['id_Departamento'].">".$tupla['id_Departamento']."</option>";
								...
								<center><button type="submit" name="enviarBD" class="btn btn-default btn-primary">Eliminar Departamento</button><br></center>
								<br>

<?php
include("configtienda.php");
if(isset($_POST['enviarBD'])){
	if(isset($_POST['id_DepartamentoE']))
	{
		$id_Departamento=$_POST['id_DepartamentoE'];
		$conexion=mysqli_connect("localhost","root","") or die("Problemas en la conexion".mysql_error());
		if($conexion)
		{
			mysqli_select_db($conexion,"tiendaropa") or die("Problemas en la seleccion de la base de datos");
			...
			else echo "Error al eliminar<br>";
			mysqli_close($conexion);
		}
	}else{
		echo'<br>';
